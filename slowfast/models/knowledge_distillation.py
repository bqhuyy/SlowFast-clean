...
class Teacher_SlowFast(nn.Module):
    ...
    def forward(self, x, bboxes=None):
        x = self.s1(x)
        x = self.s1_fuse(x)
        feature1 = [F.normalize(x[0]), F.normalize(x[1])]
        x = self.s2(x)
        x = self.s2_fuse(x)
        feature2 = [F.normalize(x[0]), F.normalize(x[1])]
        for pathway in range(self.num_pathways):
            pool = getattr(self, "pathway{}_pool".format(pathway))
            x[pathway] = pool(x[pathway])
        x = self.s3(x)
        x = self.s3_fuse(x)
        feature3 = [F.normalize(x[0]), F.normalize(x[1])]
        x = self.s4(x)
        x = self.s4_fuse(x)
        feature4 = [F.normalize(x[0]), F.normalize(x[1])]
        x = self.s5(x)
        feature5 = [F.normalize(x[0]), F.normalize(x[1])]
...
class Student_SlowFast(nn.Module):
    ...
    def forward(self, x, bboxes=None):
        ...
        if self.cfg.TRAIN.ENABLE and self.cfg.KD.ENABLE:
            kd = self.s1_kd(x)
            feature1 = [F.normalize(kd[0]), F.normalize(kd[1])]
        x = self.s2(x)
        x = self.s2_fuse(x)
        if self.cfg.TRAIN.ENABLE and self.cfg.KD.ENABLE:
            kd = self.s2_kd(x)
            feature2 = [F.normalize(kd[0]), F.normalize(kd[1])]
        for pathway in range(self.num_pathways):
            pool = getattr(self, "pathway{}_pool".format(pathway))
            x[pathway] = pool(x[pathway])
        x = self.s3(x)
        ...
        if self.cfg.TRAIN.ENABLE and self.cfg.KD.ENABLE:
            kd = self.s3_kd(x)
            feature3 = [F.normalize(kd[0]), F.normalize(kd[1])]
        x = self.s4(x)
        x = self.s4_fuse(x)
        if self.cfg.TRAIN.ENABLE and self.cfg.KD.ENABLE:
            kd = self.s4_kd(x)
            feature4 = [F.normalize(kd[0]), F.normalize(kd[1])]
        x = self.s5(x)
        if self.cfg.TRAIN.ENABLE and self.cfg.KD.ENABLE:
            kd = self.s5_kd(x)
            feature5 = [F.normalize(kd[0]), F.normalize(kd[1])]
        if self.enable_detection:
            x = self.head(x, bboxes)
        else:
            x = self.head(x)
